...
export async function POST(req: NextRequest) {
  ...
  const assignments = ranked.slice(0, 5).map((r) => {
    ...
    return {
      ...
      composite_score: r.result.compositeScore,
      trade_pts: r.result.breakdown.tradePts,
      tier_pts: r.result.breakdown.tierPts,
      geo_pts: r.result.breakdown.geoPts,
      loyalty_pts: r.result.breakdown.loyaltyPts,
      loyalty_raw: r.result.breakdown.loyaltyRaw,
