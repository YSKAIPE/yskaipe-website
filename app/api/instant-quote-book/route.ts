...
function toTradeEnum(category: string, slug: string | null | undefined): string {
  ...
  if (cat.includes('landscape') || cat.includes('landscaping') || s.includes('land_')) return 'landscaping'
...
// Generate signed claim token (48h expiry)
const CLAIM_SECRET = new TextEncoder().encode(
  process.env.CLAIM_TOKEN_SECRET ?? 'yskaipe-claim-secret-change-in-prod'
)
async function signClaimToken(jobId: string, workerId: string): Promise<string> {
...
const AGE_TIER_RANK: Record<string, number> = { junior: 0, standard: 1, adult: 2 }
const TASK_TIER_RANK: Record<string, number> = { youth: 0, primary: 1, licensed: 2 }
  ...
  try {
    ...
    const { data: jobRow, error: jobErr } = await supabase
      ...
      .insert({
        ...
        status:                'posted',
        trade:                 toTradeEnum(taskCat, slug) as any,
        age_tier_required:     taskTierMin === 'youth' ? 'junior_ok' : taskTierMin === 'licensed' ? 'adult_only' : 'standard',
        description,
        zip_code:              zip ?? null,
        task_slug:             slug ?? null,
    ...
    const unitLabel: Record<string, string> = {
      flat:'flat rate', per_hour:'per hour', per_visit:'per visit', per_sqft:'per sq ft'
    }
    await resend.emails.send({
      from: FROM, to: email,
    ...
    if (!needs_consultation && jobId) {
      await dispatchWorkers({
        jobId, confirmNumber, taskSlug: slug, taskLabel, taskCat,
...
async function dispatchWorkers({
  jobId, confirmNumber, taskSlug, taskLabel, taskCat,
  taskTierMin, taskLicReq, taskPermit,
  description, zip, timing,
  ...
  const eligible = workers.filter((w) => {
    ...
    const catMatch  = skills.some((s) =>
      s.toLowerCase().includes((taskCat ?? '').toLowerCase().split(' ')[0].toLowerCase())
    )
  ...
  if (claimsErr) console.error('[dispatch] job_claims insert error:', claimsErr)

  // Email each worker
  for (const w of targets) {
    const firstName = w.first_name ?? 'there'
    const claimToken = await signClaimToken(jobId, w.id)
    const siteUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://www.yskaipe.com'
    const claimUrl = `${siteUrl}/claim.html?token=${encodeURIComponent(claimToken)}`
    await resend.emails.send({
      from:    FROM,
      to:      w.email,
    ...
    }).catch((e) => console.error(`[dispatch] Worker email failed ${w.email}:`, e))
  }

  console.log(`[dispatch] ${confirmNumber} — notified ${targets.length} workers for "${taskLabel}"`)
}
